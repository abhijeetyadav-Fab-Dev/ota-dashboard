...
const SCRAPER_PATH = "C:\\Users\\cs03778\\hygiene_scraper.py";
const STATUS_FILE  = "C:\\Users\\cs03778\\hygiene_status.json";
const SYNC_URL     = "http://localhost:3000/api/sync-hygiene";
...
function killExistingScrapers() {
  try { execSync(`wmic process where "CommandLine like '%hygiene_scraper%'" call terminate`, { stdio: "ignore" }); } catch {}
}
...
export async function POST() {
  ...
  const child = spawn(pythonCmd, [SCRAPER_PATH], {
    env: { ...process.env, HYGIENE_STATUS_FILE: STATUS_FILE, HYGIENE_SYNC_URL: SYNC_URL, PYTHONUNBUFFERED: "1", PYTHONIOENCODING: "utf-8" },
    detached: false,
